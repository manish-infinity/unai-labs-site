...
- **BLOCKER — still blocked Sprint 36 (2026-08-15); originally re-opened Sprint 23 (2026-07-22): the wp-admin automation session has EXPIRED again; there was no FULL wp-admin access this sprint (blocked Sprints 17-20, 23-36). NOTE (Sprint 34): this sprint was a clean session-expired block — /wp-admin/edit.php redirected straight to wp-login with reauth=1 and empty fields, no partial access this time (unlike Sprint 33, when /wp-admin/upgrade.php was briefly reachable to complete a core DB update). Publishing remains blocked.** wp-login redirected with reauth=1, and entering a password is out of scope for automation. This blocks publishing Tutor lessons (so Course 2 Module 4, Course 3 Modules 1-4, and Blog Posts 10-18 are staged in GitHub, not published; see PUBLISHING_QUEUE.md for the ordered publish checklist), re-verifying WooCommerce product 120, and all other admin-side work; the Tutor course/lesson layer also cannot be verified because it is not exposed in the public REST API. Manish: log into https://unai-labs.com/wp-admin once in the automation browser profile with **Remember Me** checked, OR create a WordPress Application Password, so the session stops lapsing between sprints. (History: access was restored Sprint 21 and the whole queue cleared — posts through 135, Course 2 lessons 126-128 / 137-140 / 141-143 — then lapsed again by Sprint 23.)
...
- **Course 3 "Brain Health 101" (homepage COMING SOON card) — now CONTENT-COMPLETE IN DRAFT (all 4 modules written, Sprint 27).** Its course outline (courses/course-03-brain-health.md) and all four modules' lesson content (content/course-03-module-1-lessons.md, -module-2-lessons.md, -module-3-lessons.md, -module-4-lessons.md) are DRAFTED in GitHub but NOT in WordPress — a future authenticated sprint creates the Tutor course + Modules 1-4 lessons, sets price, mirrors product 120 with a WooCommerce product, replaces the homepage COMING SOON card with a real course card, and aligns the card's "four pillars" copy with the five-lever outline.
