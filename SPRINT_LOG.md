...
- **BLOCKER — still blocked Sprint 32 (2026-08-07); originally re-opened Sprint 23 (2026-07-22): the wp-admin automation session has EXPIRED again; there was no wp-admin access this sprint (blocked Sprints 17-20, 23-32).** wp-login redirected with reauth=1, and entering a password is out of scope for automation. This blocks publishing Tutor lessons (so Course 2 Module 4, Course 3 Modules 1-4, and Blog Posts 10-14 are staged in GitHub, not published), re-verifying WooCommerce product 120, and all other admin-side work; the Tutor course/lesson layer also cannot be verified because it is not exposed in the public REST API. Manish: log into https://unai-labs.com/wp-admin once in the automation browser profile with **Remember Me** checked, OR create a WordPress Application Password, so the session stops lapsing between sprints. (History: access was restored Sprint 21 and the whole queue cleared — posts through 135, Course 2 lessons 126-128 / 137-140 / 141-143 — then lapsed again by Sprint 23.)
