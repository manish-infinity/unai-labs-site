...
- **wp-admin access was found LIVE AGAIN and USED in Sprint 53 (2026-09-22) after ~32 consecutive blocked sprints — the existing browser-profile session was already authenticated (manish1706, administrator), so Sprint 53 published the entire 11-post blog queue (posts 177–187) via WP REST. The session is STILL NOT durable (it has lapsed 17+ times before and will likely expire again). Manish: to make it durable, log into https://unai-labs.com/wp-admin once in the automation browser profile with Remember Me checked, OR create a WordPress Application Password — otherwise every sprint risks re-blocking on publishing.
...
- **Course 4 "Deep Focus — Reclaiming Attention in the Age of AI" — LESSON CONTENT NOW COMPLETE (all 4 modules, 12 lessons written + staged in GitHub).** Outline courses/course-04-deep-focus.md (11,703 b) + Modules 1–4: content/course-04-module-1-lessons.md (16,374 b), -module-2 (21,123 b), -module-3 (22,358 b, Sprint 51), -module-4 (20,420 b, Sprint 51) — 3 lessons each, house template, cross-links only PUBLISHED posts. Still pending wp-admin: build the Tutor course + 4 module topics + 12 lessons, create a WooCommerce product (mirror product 120), add a Course 4 card to homepage/Courses. **Assessment/quiz bank now ALSO staged (Sprint 52): content/course-04-quizzes.md - 4 module knowledge-check quizzes (20 auto-graded Qs, MCQ + True/False) + a 6-item capstone self-evaluation rubric, with turnkey Tutor build instructions; build these alongside the course so Course 4 launches with assessments.** Manish: confirm price (proposed ₹2,999/$35, mirror Course 1).
...
- **Decide whether/when to produce video versions of course lessons** — Course 1 (course 30) is content-complete across all 15 lessons (text), but its page promises "video + reading + exercises."
